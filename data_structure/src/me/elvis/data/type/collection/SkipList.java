...
public class SkipList<K extends Comparable<K>, V> implements Iterable<K> {
    ...
    public static void simpleDemoTest() {
    ...
    public static void simplePerfTest() {
        ConcurrentSkipListTest.generateData(false);
        List<Integer> dataList = ConcurrentSkipListTest.dataList;
        SkipList<Integer, String> skipList = new SkipList<>();
        long startTime = System.currentTimeMillis();
        for (Integer data : dataList) {
            skipList.add(data, String.valueOf(data));
        }
        long endTime = System.currentTimeMillis();
        long duration = endTime - startTime;
        System.out.println(duration);

        List<SkipList<Integer, String>> lists = new ArrayList<>();
        int listNum = 100;
        for (int i = 0; i < listNum; i++) {
            lists.add(new SkipList<>());
        }
        startTime = System.currentTimeMillis();
        for (Integer data : dataList) {
            SkipList<Integer, String> singleList = lists.get(data % listNum);
            skipList.add(data, String.valueOf(data));
        }
